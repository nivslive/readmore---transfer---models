...
class Proposals_model extends App_Model
{
    ...
    public function INCREMENT_NEXT_INVOICE_NUMBER_IN_OPTIONS()
    {
        $invoice_model = new Invoices_model;
        $invoice_model->increment_next_number();
    }


    public function AUTOMATIONS($proposal_id, $optionalData = null) 
    {
        if(!$this->DESATIVATE_ALL_AUTOMATION) {
            
                // AUTOMATIZAR A FATURA
                // AO TER STATUS 3 - ACEITO
                // JÁ CRIA AUTOMATICAMENTE UMA FATURA.
                $this->automate_create_invoice_if_proposal_status_has_3($proposal_id, $optionalData);

                // AUTOMATIZAR A CONVERSÃO DO LEAD PARA CLIENTE
                // AO TER STATUS 3 - ACEITO
                // E SER UM LEAD
                // O LEAD SE TORNA CLIENTE
                // $this->automate_convert_lead_to_customer_if_proposal_is_status_3($proposal_id);
                // TEVE QUE SER INCLUIDO DENTRO DE $this->automate_create_invoice_if_proposal_status_has_3
                // pois necessita de uma validação e de um update com o proposal 
...
//        var_dump($model);die();
    ...
    public function automate_create_invoice_if_proposal_status_has_3($id, $optionalData = null)
    {
        ...
        if(isset($proposal_data) && $proposal_data['status'] === '3')
        {
            ...
            $invoice_data = [
                ...
                'clientnote' => isset($proposal_data['content']) ? $proposal_data['content'] : '',  // Verifique se existe o índice 'content'
                'project_id' => $proposal_data['project_id'] ?? null,
                'billing_street' => $proposal_data['address'],
            ...
            $proposals_update_data['invoice_id'] = $invoice_id;
    ...
    public function mark_action_status($status, $id, $client = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // TODAS AS AUTOMATIZAÇÕES DE PROPOSALS MODELS ESTÃO DENTRO DESSA FUNÇÃO
            // ENTRE NELA E LEIA MAIS
            $this->AUTOMATIONS($id);
