...
class Leads_model extends App_Model
{
     ...
     */
    public function create_password_customer_by_existent_lead_and_send_email() {
        $lead = $this->db->where('id', $client['leadid'])->get(db_prefix().'leads')->row_array();
        if (!empty($lead['email'])) {
            $this->load->model('authentication_model');
            $this->authentication_model->set_password_email($lead['email'], 0);
        }
    }
    // public function create_contact_by_existent_customer_and_send_email($id)
    // {
    //     $client = $this->db->where('userid', $id)->get(db_prefix().'clients')->row_array();
    //     $lead = $this->db->where('id', $client['leadid'])->get(db_prefix().'leads')->row_array();
    //     //echo "<pre>";var_dump($lead);die();
    //     $contact_data['email'] = $lead['email'];
    //     $contact_data['firstname'] = $lead['name'];
    //     $contact_data['phonenumber'] = $lead['phonenumber'];

    //     $contact = $this->db->where('userid', $client['userid'])->get(db_prefix().'contacts');
        
    //     if($contact->count_rows() === 0) {
    //         $contact = $contact->row();
    ...
    //             $contact = $this->db->where('id', $contacts_id)->get(db_prefix().'contacts')->row();
    //             $data['new_pass_key'] = app_generate_hash();
    //             $data['userid']    = $contacts_id;
    //             $data['email']   = "test@gmail.com";
    //             $sent = send_mail_template('customer_contact_set_password', $contact, $data);
    //             if($sent)
    //             {
    //                 return ['contact_already_exists' => false, 'email_sended' => true, 'id' => $contacts_id];
    //             }
    //         }
    //         return ['contact_already_exists' => false, 'email_sended' => false, 'id' => $contacts_id];
    //     }

    //     return ['contact_already_exists' => true, 'email_sended' => false, 'id' => null];
    // }

    public function convert_to_customer_by_existent_lead($id)
    {
        ...
        if($client->num_rows() === 0) {
            ...
            $data = [];
            $data['billing_street']  = $lead['address'];
            $data['billing_city']    = $lead['city'];
            $data['billing_state']   = $lead['state'];
            $data['billing_zip']     = $lead['zip'];
            $data['billing_country'] = $lead['country'];
            ...
            // {
            //     //var_dump($contact->row());die();
            //     $contact_id = $contact->row()->id;
            //     $this->db->where('id', $new_client_id);
            //     $this->db->update(db_prefix() . 'clients', [
            //         'show_primary_contact' => $contact_id,
            //     ]);
            // }
            ...
            // enviar email para criar o password
            $this->load->model('authentication_model');
            $mail = $this->authentication_model->set_password_email($lead['email'], 0);

            if(is_int($new_client_id))
            {
